...
function ReportsPage() {
  ...
  async function fetchInitialData() {
    try {
      ...
      const { data: cycle } = await supabase.from("cycles").select("*").eq("status", "in_progress").single();
      if (cycle) {
        setActiveCycle(cycle);
        const { data: week } = await supabase.from("weeks").select("*").eq("cycle_id", cycle.id).order("number", { ascending: true }).limit(1).single();
        if (week) setActiveWeek(week);
      }

      const { data: summaries } = await supabase.from("annual_report_summary").select("*").order("year", { ascending: false });
      if (summaries) setAnnualSummaries(summaries);
    } catch (error) {
  ...
  return (
    <LandscapeBulletinLayout
      ...
      subtitle="Resumo operacional automático"
      agentInfo={{
        ...
        block: "--",
        street: "Consolidado Semanal"
      }}
      stats={{
        worked: currentBulletin.propertiesWorked,
        total: 500,
        closed: 18,
        refused: 4,
    ...
    >
      <div className={cn("space-y-6 pb-8 animate-in fade-in slide-in-from-bottom-4 duration-700", isLandscape && "pb-0 h-full overflow-y-auto no-scrollbar")}>
        {!isLandscape && (
          <>
