  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      <div className="flex flex-col gap-1">
        <h2 className="text-3xl font-black tracking-tighter text-primary uppercase">Painel de Ciclos</h2>
        <p className="text-muted-foreground font-medium uppercase tracking-widest text-[10px] font-mono">Gestão Territorial de Endemias</p>
      </div>

      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <div className="h-12 w-12 border-4 border-primary border-t-transparent rounded-full animate-spin" />
          <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">Sincronizando períodos...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6">
          {cycles.map((cycle) => {
            const coverage = coverageData[cycle.id] || { coverage_percentage: 0, worked_properties: 0, total_properties: 0 };
            ...
            return (
              <Card key={cycle.id} className={cn(
                ...
                isActive && "ring-2 ring-primary/20 scale-[1.02]"
              )}>
                <CardHeader className="pb-4">
                  <div className="flex justify-between items-start">
                    <div className={cn(
                      ...
                      isCompleted ? "bg-emerald-100 text-emerald-700 shadow-emerald-100" : 
                    ...
                    <Badge variant={isActive ? 'default' : 'secondary'} className={cn(
                      "rounded-lg font-black text-[9px] uppercase tracking-widest border-none px-3 py-1",
                ...
                <CardContent className="pt-2 pb-6 space-y-5">
                  <div className="space-y-2">
                    <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                      ...
                      <span className={cn(isActive && "text-primary")}>{coverage.coverage_percentage}%</span>
                    </div>
                    <Progress value={coverage.coverage_percentage} className={cn("h-2 rounded-full", isActive ? "bg-primary/10" : "bg-slate-100")} />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <CycleStatCard label="Imóveis" value={coverage.worked_properties} icon={Home} color="text-blue-600" bgColor="bg-blue-50" />
                    <CycleStatCard label="Focos" value={isActive ? 12 : 0} icon={AlertTriangle} color="text-red-600" bgColor="bg-red-50" />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <CycleStatCard label="Fechados" value={isActive ? 18 : 0} icon={XCircle} color="text-orange-600" bgColor="bg-orange-50" />
                    <CycleStatCard label="Recusados" value={isActive ? 4 : 0} icon={AlertTriangle} color="text-rose-600" bgColor="bg-rose-50" />
                  </div>

                  <Button 
                    onClick={() => handleGenerateReport(cycle)}
                    className={cn(
                      "w-full h-14 rounded-2xl font-black uppercase tracking-widest text-[10px] gap-2 border-none transition-all shadow-lg active:scale-95",
                  ...
                  >
                    <FileText className="h-4 w-4" /> Gerar Relatório do Ciclo
                  </Button>
                </CardContent>
              </Card>
            );
...
function CycleStatCard({ label, value, icon: Icon, color, bgColor }: any) {
  return (
    <div className={cn("flex flex-col p-4 rounded-3xl border border-slate-50 shadow-sm", bgColor)}>
      <div className="flex items-center justify-between mb-2">
        <div className={cn("p-1.5 rounded-lg bg-white/50", color)}>
          <Icon className="h-3.5 w-3.5" />
        </div>
        <span className="text-xl font-black tracking-tight text-slate-800">{value}</span>
      </div>
